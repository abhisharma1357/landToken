...
contract LorcPreSale is Ownable, Pausable {
  ...
  function buyWithETH() public payable whenNotPaused returns(bool res) {
    ...
    lorcToken.transfer(msg.sender, lorcValue);
    emit BuyLORCWithETH(msg.sender, address(lorcToken), lorcValue, ethRate);
    return true;
  }

  /// @dev buy LORC with USDT
  function buyWithUSDT(uint _usdtAmount) public whenNotPaused returns(bool res) {
  ...
  function withdrawETH (uint _amount) public onlyOwner returns(bool res) {
  ...
  function withdrawErc20(IERC20 token) public onlyOwner returns(bool res) {
    uint256 _balance = token.balanceOf(address(this));
    require(_balance > 0, "LorcPreSale: insufficient ERC20 balance");
    require(token.transfer(msg.sender, _balance), "LorcPreSale: Transfer failed");
    return true;
  }
